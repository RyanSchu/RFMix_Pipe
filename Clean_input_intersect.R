...
if(nIDs != nBPs){
  ...
  if(nIDs/nBPs <= 0.90 | nIDs/nBPs >= 1/(0.90)){
    ...
    cat("Continuing with smallest number of snps")
  }  
}

final_list<-inner_join(refSNPS,querySNPS,by=c("ID","bp")) %>% inner_join(mapSNPS,by=c("ID","bp"))
fwrite(final_list,args$out %&% "_intersect_snp.list.txt",sep="\t",colnames=F)
